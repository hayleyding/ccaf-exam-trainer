...
**1.5 Agent SDK hooks for interception and normalization.** `PostToolUse` hooks transform tool results before the model sees them (e.g., normalise Unix vs ISO 8601 timestamps, numeric status codes). Interception hooks block policy-violating calls (e.g., refunds over $500) and redirect to escalation. Choose hooks over prompts when business rules require guaranteed compliance.
